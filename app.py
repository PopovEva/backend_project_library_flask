...
def get_loans():
    try:
        ...
        for loan in loans:
            book = loan.book
            loan_period = "up to 10 days" if book.type == 1 else "up to 5 days" if book.type == 2 else "up to 2 days"
            loan_data = {
                'user': {
...
@app.route('/book-status/<int:book_id>', methods=['PUT'])
@admin_required
def update_book_status(book_id):
    try:
        ...
        if book.is_active == is_active:
            ...
            return jsonify({"message": f"Book is already {action}"}), 400
